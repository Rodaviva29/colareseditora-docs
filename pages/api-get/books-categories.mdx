...
<Tabs items={['HTTP', 'Curl', 'NodeJS', 'Python']} defaultIndex="1">
  <Tabs.Tab>
  ```http copy
    localhost:5000/api/v1/books/categories/Java?page=1&limit=10
  ```
  </Tabs.Tab>
  <Tabs.Tab>
  ``` curl copy
    curl -X GET 'localhost:5000/api/v1/books/categories/Java?page=1&limit=10'
  ```
  </Tabs.Tab>
  <Tabs.Tab>
  ```js copy
    ...
    axios.get('localhost:5000/api/v1/books/categories/Java', {
    params: {
        page: 1,
        limit: 10
    }
    ...
    url = 'localhost:5000/api/v1/books/categories/Java'
    params = {
        'page': 1,
        'limit': 10
    }
