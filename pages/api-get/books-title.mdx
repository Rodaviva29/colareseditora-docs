...
<Tabs items={['HTTP', 'Curl', 'NodeJS', 'Python']} defaultIndex="1">
  <Tabs.Tab>
  ```http copy
    localhost:5000/api/v1/books/title/Flex?page=1&limit=10
  ```
  </Tabs.Tab>
  <Tabs.Tab>
  ```curl copy
    curl -X GET 'localhost:5000/api/v1/books/title/Flex?page=1&limit=10'
  ```
  </Tabs.Tab>
  ...
  <Tabs.Tab>
    ...
    axios.get('localhost:5000/api/v1/books/title/Flex', {
    params: {
        page: 1,
        limit: 10
    }
  ...
  <Tabs.Tab>
    ...
    url = 'localhost:5000/api/v1/books/title/Flex'
    params = {
        'page': 1,
        'limit': 10
    }
