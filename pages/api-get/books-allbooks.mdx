...
<Tabs items={['HTTP', 'Curl', 'NodeJS', 'Python']} defaultIndex="1">
  <Tabs.Tab>
  ```http copy
    localhost:5000/api/v1/books/allbooks
  ```
  </Tabs.Tab>
  <Tabs.Tab>
  ``` curl copy
    curl -X GET 'localhost:5000/api/v1/books/allbooks'
  ```
  </Tabs.Tab>
  <Tabs.Tab>
  ```js copy
    ...
    axios.get('localhost:5000/api/v1/books/allbooks')
    .then((response) => {
    ...
    url = 'localhost:5000/api/v1/books/allbooks'
